...
app = FastAPI(title="AI Clothing Shopping Assistant API", version="0.2.0")

FASHION200K_IMAGE_DIR = (
    Path(__file__).resolve().parents[1] / "data" / "fashion200k-images"
)

app.mount(
    "/api/fashion200k-images",
    StaticFiles(directory=FASHION200K_IMAGE_DIR),
    name="fashion200k-images",
)
app.mount(
    "/api/try-on-files",
...
def health() -> HealthResponse:
    return HealthResponse(status="ok", product_count=database_product_count())
...
def product_detail(product_id: str) -> ProductDetailResponse:
    ...
    demo_fields = [
        label
        for label, source in (
            ("价格", product.price_source),
            ("尺码", product.size_source),
            ("季节", product.season_source),
            ("库存", product.stock_source),
        )
        if source == "synthetic_demo"
    ]
    notice = (
        "、".join(demo_fields) + "为演示数据，仅用于门店流程展示。"
        if demo_fields
        else "商品信息来自当前项目商品库。"
    )
    return ProductDetailResponse(product=product, data_notice=notice)
...
def cart_detail(profile_id: str) -> CartResponse:
    return get_cart(profile_id, get_product_by_id)
...
def cart_add_item(
    ...
    cart = get_cart(profile_id, get_product_by_id)
    ...
    if current_quantity + request.quantity > 99:
        raise HTTPException(status_code=400, detail="单件商品最多添加 99 件")
...
def cart_remove_item(
    ...
    return remove_cart_item(profile_id, product_id, selected_size, get_product_by_id)


@app.post("/api/orders", response_model=DemoOrder)
def order_create(request: OrderCreateRequest) -> DemoOrder:
    cart = get_cart(request.profile_id, get_product_by_id)
    if not cart.items:
        raise HTTPException(status_code=400, detail="演示购物车为空")
    return create_demo_order(request.profile_id, get_product_by_id)
