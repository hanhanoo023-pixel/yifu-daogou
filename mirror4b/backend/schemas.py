...
class Mirror4BProduct(BaseModel):
    sku: str
    name: str
    main_image_url: str
...
class Mirror4BHistoryMessage(BaseModel):
    role: str
    content: str


class Mirror4BAgentRuntimeContext(BaseModel):
    has_person_photo: bool = False
    ...
    recommended_outfits: list[dict[str, Any]] = Field(default_factory=list)


class Mirror4BAgentChatRequest(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)

    text: str = Field(min_length=1, max_length=1000)
    current_page: str | None = None
    history: list[Mirror4BHistoryMessage] | None = None
    excluded_product_ids: list[int] = Field(default_factory=list)
    context: Mirror4BAgentRuntimeContext = Field(
        default_factory=Mirror4BAgentRuntimeContext
    )


class Mirror4BAgentActionPayload(BaseModel):
    route_name: str | None = None
    intent: str | None = None
    parameters: dict[str, Any] = Field(default_factory=dict)


class Mirror4BAgentActionResponse(BaseModel):
    type: str
    payload: Mirror4BAgentActionPayload | None = None


class Mirror4BAgentMessage(BaseModel):
    text: str
    action: Mirror4BAgentActionResponse | None = None
    intent: str
    sales_stage: str
    recommended_products: list[dict[str, Any]] | None = None
    recommended_outfits: list[dict[str, Any]] | None = None
    recommendation_method: str | None = None
    debug_note: str | None = None
    quick_replies: list[str] = Field(default_factory=list)


class Mirror4BAgentChatResponse(BaseModel):
    status: str = "SUCCESS"
    message: Mirror4BAgentMessage
