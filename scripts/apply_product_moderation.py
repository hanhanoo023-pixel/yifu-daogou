...
def main() -> None:
    ...
    if existing_count != len(product_ids):
        raise RuntimeError(
            f"expected {len(product_ids)} products in database, found {existing_count}"
        )
    ...
    connection.execute(
        ...
        CREATE TABLE product_moderation (
            product_id TEXT PRIMARY KEY REFERENCES products(parent_asin),
            audit_version TEXT NOT NULL,
            status TEXT NOT NULL CHECK (status IN ('FAIL')),
